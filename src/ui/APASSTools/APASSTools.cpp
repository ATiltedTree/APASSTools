...
APASSTools::APASSTools(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::APASSTools(this)) {
  ui->setupUi();
  ...
  this->ui->observationSpin->setValue(
      this->settings.appSettings.observationThreshold);
  this->ui->magnitudeSpin->setValue(
      this->settings.appSettings.magnitudeThreshold);
}
...
void APASSTools::closeEvent(QCloseEvent *event) {
  if (this->unsavedChanges) {
    ...
    askForSave.setStandardButtons(QMessageBox::Save | QMessageBox::Cancel |
                                  QMessageBox::Discard);
...
void APASSTools::setupConnections() {
  connect(this->ui->actionAboutQt, &QAction::triggered, this,
          &QApplication::aboutQt);
  connect(this->ui->actionAbout, &QAction::triggered, this,
          &APASSTools::onAbout);
  connect(this->ui->actionQuit, &QAction::triggered, this, &APASSTools::close);
  connect(this->ui->actionCSV, &QAction::triggered, this,
          &APASSTools::onFromCSVFile);
  connect(this->ui->actionWeb, &QAction::triggered, this,
          &APASSTools::onFromWeb);
  connect(this->ui->actionClear, &QAction::triggered, this,
          &APASSTools::onClear);
  connect(this->ui->actionSettings, &QAction::triggered, this,
          &APASSTools::onSettings);
  connect(this->ui->actionSave, &QAction::triggered, this, &APASSTools::onSave);
  connect(this->ui->actionSaveAs, &QAction::triggered, this,
          &APASSTools::onSaveAs);
}
...
void APASSTools::doImport(const QString &data) {
  ...
  this->apass.importCSV(data, this->ui->observationSpin->value(),
                        this->ui->magnitudeSpin->value(), bar);
  this->updateTree();
...
void APASSTools::doSave(bool saveAs) {
  ...
  if (!this->unsavedChanges) {
    QMessageBox::information(this, "Error!",
                             "Please import something before saving !");
  }
  QString dirname;
